<%@ Page Language="vb" AutoEventWireup="false" MasterPageFile="~/MasterPage.master" CodeBehind="divpopup.aspx.vb" Inherits="DansTools.divpopup" 
                ...
                <Columns>
                    <asp:TemplateField ShowHeader="False">
                        <ItemTemplate>
                            ...
                            <input type="button" class="form" onclick="javascript:ShowPopup('<%# Eval("Id")%>','<%# Eval("Text")%>');" value="Edit"></ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="MenuID" InsertVisible="False" SortExpression="MenuID">
                        <ItemTemplate>
                            <asp:Label ID="MenuID" runat="server" Text='<%# Bind("Id")%>'></asp:Label>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="MenuText" SortExpression="MenuText">
                        <ItemTemplate>
                            <asp:Label ID="MenuText" runat="server" Text='<%# Bind("Text")%>'></asp:Label>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </asp:GridView>
            <asp:SqlDataSource ID="dscTest" runat="server" ConnectionString="<%$ ConnectionStrings:PPPConnectionString %>"
                SelectCommand="SELECT [Id], [Text] FROM [tblMenu] ORDER BY [MenuOrder]">
            </asp:SqlDataSource>
